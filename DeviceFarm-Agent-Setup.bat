...
"%PS%" -NoProfile -ExecutionPolicy Bypass -Command ^
    "$okIpv4 = $false; $okIpv6 = $false; for ($i = 0; $i -lt 15; $i++) { try { $r4 = Invoke-WebRequest -Uri 'http://127.0.0.1:7400/api/binding/code' -UseBasicParsing -TimeoutSec 2; if ($r4.StatusCode -eq 200) { $okIpv4 = $true } } catch {}; try { $r6 = Invoke-WebRequest -Uri 'http://[::1]:7400/api/binding/code' -UseBasicParsing -TimeoutSec 2; if ($r6.StatusCode -eq 200) { $okIpv6 = $true } } catch {}; if ($okIpv4 -or $okIpv6) { break }; Start-Sleep -Seconds 1 }; if ($okIpv4) { Write-Host ' [OK] Local Dashboard Server is LIVE on IPv4 (127.0.0.1:7400)!' } else { Write-Host ' [*] Local Dashboard Server IPv4 pending...' }; if ($okIpv6) { Write-Host ' [OK] Local Dashboard Server is LIVE on IPv6 ([::1]:7400)!' } else { Write-Host ' [*] Local Dashboard Server IPv6 pending...' }; $cf = Get-Process -Name 'cloudflared' -ErrorAction SilentlyContinue; if ($cf) { Write-Host (' [OK] Cloudflare Tunnel daemon is RUNNING (PID: ' + $cf[0].Id + ')') } else { Write-Host ' [WARN] Cloudflare Tunnel process not detected' }; $okPub = $false; for ($j = 0; $j -lt 10; $j++) { try { $rp = Invoke-WebRequest -Uri 'https://agent.dennoh.site/api/binding/code' -UseBasicParsing -TimeoutSec 3; if ($rp.StatusCode -eq 200) { $okPub = $true; break } } catch {}; Start-Sleep -Seconds 1 }; if ($okPub) { Write-Host ' [OK] Public URL https://agent.dennoh.site is LIVE and responding 200 OK!' } else { Write-Host ' [*] Public tunnel route is propagating...' }"
